#### Experimental version(not ready for deployment).

### Working well for all(tuned parameters) : python quality_image_emotion_detection_facial_alignment.py --shape_predictor ./models/shape_predictor_5_face_landmarks.dat --position_left_eye_X 0.3 --position_left_eye_Y 0.47 --image_width 600 --image_height 780 --input_path 'images/*.*' --aligned_faces_path 'aligned_faces/' --undetected_faces_path 'face_not_detected/'

# Command -> python quality_image_emotion_detection_facial_alignment.py --shape_predictor ./models/shape_predictor_5_face_landmarks.dat --position_left_eye_X 0.3 --position_left_eye_Y 0.3 --image_width 300 --image_height 300 --input_path 'images/*.*' --aligned_faces_path 'aligned_faces/' --undetected_faces_path 'face_not_detected/'
# Author : Subhajit Das.
import cv2
import numpy as np
from keras.models import model_from_json
from keras.preprocessing import image as image_kr
    ...
    if(Height >= 800 and Width >= 800):
        ...
        if (len(rects)== 0):
            ...
            for rect in rects:
                ...
                (xs, ys, ws, hs) = rect_to_bb(rect)
                for (x,y,w,h) in [(xs, ys, ws, hs)]:
                        roi_gray=gray[y:y+w,x:x+h]#cropping region of interest i.e. face area from  image
                        roi_gray=cv2.resize(roi_gray,(48,48))
                        img_pixels = image_kr.img_to_array(roi_gray)
                ...
                if(len(rects) == 1):
                    ...
                    cv2.imwrite(aligned_faces + predicted_emotion + "/" + f_2, face_aligned)
                # For the images in which multiple faces were detected    
                else:
                    m = str(re.sub(r'.*/', '/', file))
                    l = list(m) # convert to list
                    ...
                    del(l[p]) # delete it
                    f_2 = "".join(l) # convert back to string
                    cv2.imwrite(aligned_faces + predicted_emotion + "/" + str(i)+"_"+f_2, face_aligned)
                i += 1
